...
#' @param CECAL_ONLY c(TRUE,FALSE), if TRUE will only use cecal contents for tpm and prevalence filtering 
...
#Testing for meanSdPlots, QQ (negative binomial)
```{r}
...
#   # qqline(expr_vec)
#   ks_test <- ks.test(expr_vec,'pnorm',mean=expr_mean,sd=expr_std)
#   ks_p <- ks_test$p.value
#   ks_D <- ks_test$statistic
#   print(ks_p)
#   print(ks_D)
# }
# ks.test(expr_vec,'pnorm',mean=expr_mean,sd=expr_std)
...
CECAL_ONLY = TRUE
if (CECAL_ONLY) {
  # prevotella_samples <- grep(PREVOTELLA_COL_RE, colnames(tpm.df)) #example code
  #prevotella.df <- select(tpm.df,prevotella_samples)
  CECAL_COL_RE = "cecal"
  cecal_samples <- grep(CECAL_COL_RE, colnames(filt_rlog.df)) 
...
#MeanSD hex bin plots   
meanSdPlot(data.matrix(np_filt_vst.df),ranks=TRUE)
meanSdPlot(data.matrix(np_filt_log.df),ranks=TRUE)
meanSdPlot(data.matrix(np_filt_rlog.df),ranks=TRUE)

np_filt_vst.vec <- unlist(np_filt_vst.df)
np_filt_log.vec <- unlist(np_filt_log.df)




np_filt_rlog.vec <- unlist(np_filt_rlog.df)

#QQ Plots - NB distribution 
library(MASS)
mode(np_filt_rlog.vec) <- "integer"
head(np_filt_rlog.vec)
print(min(np_filt_rlog.vec))
np_filt_rlog.vec <- np_filt_rlog.vec - min(np_filt_rlog.vec)
head(np_filt_rlog.vec)
params = fitdistr(np_filt_rlog.vec,"Negative Binomial")
fit_size <- params$estimate[[1]]
fit_mu <- params$estimate[[2]]
np_filt_rlog.s = sort(np_filt_rlog.vec)
n <- length(np_filt_rlog.vec)
p <- ppoints(n)
np_filt_rlog.q = qnbinom(ppoints(np_filt_rlog.vec),size=fit_size,mu=fit_mu)
plot(np_filt_rlog.q, np_filt_rlog.s, col="red", xlab="Negative Binomial theoretical quantiles", ylab="Data quantiles")
abline(0,1)
```
...
print(params$estimate[[1]])
print(params$estimate[[2]])
```
