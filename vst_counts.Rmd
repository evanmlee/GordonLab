...
meanSD_plot <- function(filt_expr.df,expr_type,ranks=TRUE) {
  ...
  title = sprintf("Mean Standard Deviation Plot, %s expression data",expr_type)
  msp <- msp + labs(title=title,x="Rank(mean)",y="Standard Deviation") + theme(plot.title = element_text(hjust = 0.5))
  return(msp)
}
...
filt_qq_plot <- function(filt_expr.df,expr_type) {
...
#MeanSD hex bin plots; patchwork combine   
...
combinged_meansd <- vst_msp + log_msp + rlog_msp + plot_layout(ncol = 3, guides = "collect")
